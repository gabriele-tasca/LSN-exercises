...
void Input(void)
{
  ...
  cout << "Interatomic potential v(r) = 4 * [(1/r)^12 - (1/r)^6]" << endl
       << endl;
  cout << "Boltzmann weight exp(- bbeta * sum_{i<j} v(r_ij) ), bbeta = 1/T " << endl
       << endl;
...
void Move(void)
{
  ...
  for (int i = 0; i < npart; ++i)
  {
    ...
    //Metropolis test
    p = exp(bbeta * (energy_old - energy_new));
    if (p >= rnd.Rannyu())
    {
      //Update
      x[o] = xnew;
...
void Measure()
{
  ...
  for (int i = 0; i < npart - 1; ++i)
  {
    for (int j = i + 1; j < npart; ++j)
    {
      ...
      for (int jbin = igofr; jbin < igofr + nbins; ++jbin)
...
void Averages(int iblk) //Print results for current block
{
  ...
  //"stima_g(r)" isn't a thing and the calculation is done in-place because this is confusing enough already i think
  for (int k = igofr; k < igofr + nbins; ++k)
  {
    double _r = (k - igofr) * bin_size;
    double dvr = 4. * pi / 3. * (   pow(_r + bin_size, 3) - pow(_r, 3)  );
    blk_av[k] = blk_av[k] / (rho * npart * dvr * nblk * blk_norm);
    glob_av[k] += blk_av[k];
    glob_av2[k] += blk_av[k] * blk_av[k];
    double kth_gr_err = Error(glob_av[k], glob_av2[k], iblk);
    Gofr << iblk << "   " << _r << "   " << glob_av[k] << "   " << kth_gr_err << "\n";
  }
  Gofr << endl
       << endl
       << endl;
...
void ConfFinal(void)
{
  ...
  ofstream Gave; //print final values in separate file
  Gave.open("output.gave.0", ios::app);

  for (int k = igofr; k < igofr + nbins; ++k)
  {
    double _r = (k - igofr) * bin_size;
    double kth_gr_err = Error(glob_av[k], glob_av2[k], nblk);
    Gave << _r << "   " << glob_av[k] << "    " << kth_gr_err << "\n";
  }
