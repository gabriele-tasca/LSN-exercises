...
int main(int argc, char *argv[])
{
   ...
   auto mod_psi_1s = [](vector<double> x) {   //squared modulus!
      double r = 0;
      for (int i = 0; i < 3; i++)
      {
         r += pow(x[i], 2);
   ...
   // auto mod_psi_2p = [](vector<double> x) {
   //    double r = 0;
   //    for(int i=0; i<3;i++){
   //       r += pow(x[i],2);
   //    }
   //    //double theta = acos(x[3]/r);
   //    return pow(M_E, -r)*pow(x[3]/r,2);
   // };
   ...
   for (int i = 0; i < n_blocks; i++)
   {
      ...
      for (int j = 0; j < n_throws; j++)
      {
         ...
         double alfa = min(1., mod_psi_1s(xnew) / mod_psi_1s(x));
