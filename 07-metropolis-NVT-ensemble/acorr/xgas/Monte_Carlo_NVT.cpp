...
{ 
  ...
  for (int keq=1; keq<nequil; keq++) //Equilibration
  {
    Move();
    cout << keq << endl;
  }
  cout << "END EQUILIBRATION" << endl;
...
void Input(void)
{
  ...
  cout << "Boltzmann weight exp(- bbeta * sum_{i<j} v(r_ij) ), bbeta = 1/T " << endl << endl;
...
// contribution to energy and virial
  ...
  ofstream Epot_frames; 
  ofstream Pres_frames; 

  Epot_frames.open("output.epot.frames",ios::app);
  Pres_frames.open("output.pres.frames",ios::app);
  ...
  Epot_frames << stima_pot << endl;
  Pres_frames << stima_pres << endl;
